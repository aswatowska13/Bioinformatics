<!doctype html>

<html lang="en">
<head>
  <meta charset="utf-8">

  <title>The HTML5 Herald</title>
  <meta name="description" content="The HTML5 Herald">
  <meta name="author" content="SitePoint">

  <link rel="stylesheet" href="style.css">
  <link rel="stylesheet" href="graphs.css">
  <link rel="stylesheet" href="navBar.css">
  <link rel="stylesheet" href="sidebar.css">

</head>

<body>

  <div class="grid">

    <div class="NavBar">
<<<<<<< HEAD
      <div class="navBarButton1">Home</div>
      <div class="navBarButton2">About</div>
      <div class="navBarButton3">Application</div>
=======
      <div class="navBarButton1"></div>
      <div class="navBarButton2"></div>
      <div class="navBarButton3"></div>
>>>>>>> e823f539e28098b71ee7f844fd14a697e5d8d2ec
      <div class="navBarButton4"></div>
      <div class="navBarButton5"></div>
    </div>

    <div class="sidebar">
<<<<<<< HEAD
      <div class="function1" onclick="connectDatabase()">Connect</div>
      <div class="function2" ><input type="file"
      id="dbfile" name="file"
      accept=".csv"></div>
=======
      <div class="function1" onclick="connectDatabase()"></div>
      <div class="function2"></div>
>>>>>>> e823f539e28098b71ee7f844fd14a697e5d8d2ec
      <div class="function3"></div>
      <div class="function4"></div>
      <div class="function5"></div>
    </div>

    <div class="graphs">
          <div class="graph1">1</div>
          <div class="graph2">2</div>
          <div class="graph3">3</div>
          <div class="graph4">4</div>
    </div>

<<<<<<< HEAD
    <div class="footer">Contact us:</div>

  </div>
  <script src="dataBase/connectSQL.js"></script>
  <script src="dataBase/Database.js"></script>
=======
    <div class="footer">Footer</div>

  </div>
  <script src="dataBase/connectSQL.js"></script>
>>>>>>> e823f539e28098b71ee7f844fd14a697e5d8d2ec
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
</body>
</html>
